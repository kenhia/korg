...
### The published read-shape contract (2041)

Everything above this line is fenced **inside korg**. That was enough for
korg's own web app and for an agent reading these docs, and it reached the two
consumers that had copied korg's sink not at all. When the list reads grew
envelopes, kmon's copy crashed daily for 11 days (WI 899) and kyac's silently
wrote nothing for two months (kyac sprint 020) — the same change, the same
contract, two repos that learned about it from their own wreckage.

So korg publishes the shapes:

| | |
|---|---|
| **The document** | [`contract/read-shapes.json`](../contract/read-shapes.json) |
| **Over HTTP** | `GET /api/contract/read-shapes` |
| **Generated by** | `crates/korg-mcp/tests/read_shapes.rs` |
| **Regenerate** | `UPDATE_READ_SHAPES=1 cargo test -p korg-mcp --test read_shapes` (or `just gen`) |

Per read it states the shape class, the envelope keys, the `omitted` keys, the
keys every row carries, the keys **some** rows carry, and — where korg has a
fuller read for those rows — what that read adds and what only the list
computes. `only_on_full_read` is the field the silent half of this failure
needed: a lean work-item row carries no `tags`, and a dedup built on them reads
`null` forever without raising.

Four properties make it a contract rather than a description, and each is
enforced:

- **Measured, not written.** Every value is read off a real dispatched
  response. A hand-written contract would be a second source of truth, which
  is the thing WI 2041 is about.
- **Exhaustive by assertion.** The covered set is compared with the live
  advertised tool list, so a new collection read fails the build until it is
  published.
- **Drift-gated.** `contract/read-shapes.json` is in `just gen`'s fingerprint,
  so a shape change that does not regenerate it fails `just check` and CI.
- **Honest about variance** (korg+ GP-14). Key presence is measured across
  rows deliberately seeded to disagree, because serde omits a key whose value
  would be meaningless rather than sending `null` —
  `ProjectLeanRow::status` is the live instance, absent on an active row and
  present on every other. A guard keeps that specimen pinned: a seed that
  loses the variance fails even though the regenerated document honestly
  matches it.

**For a consumer.** Vendor the document and assert against it in your own CI;
that is the whole intent, and it turns a korg shape change into a red build in
your repo instead of a production failure in korg's. Fetch it from the korg you
actually talk to when you want to know whether what you vendored is still
current — the endpoint serves the file byte-for-byte, so hashing the two is the
entire staleness check. What it does **not** do is describe write payloads or
single-node shapes beyond the full-read counterparts; those are the tool
schemas' job (`initialize`, or `crates/korg-mcp/tests/tools_schema.json`).
