...
pub fn tools() -> Vec<Tool> {
    vec![
        tool::<NewWorkItem>("create_work_item", "Create a work item. Returns the created row (including node_id and the serial wi_number, which are the same number since the 0009 identity migration). Two SOAK fields (korg #2153) travel on this item and matter only for extended tests: `check_after` (a `YYYY-MM-DD` date -- the earliest the item's evidence can be judged) and `invalidated_if` (free text -- what state, if it changes, VOIDS the test). Both are null on ordinary work and both are REQUIRED before a program may `soaks` this item. Write `invalidated_if` as a condition somebody could check, not a hope: \"invalidated if kai's baseline is regenerated before the 09-10 nightly\" is the shape, and it is the whole difference between a soak that is watched and one that quietly rots."),
        tool::<ops::ListWorkItems>("list_work_items", "The work-item list: lean rows (wi_number, node_id, project, title, wi_type, wi_status, wi_tshirt, comment_count, has_details, has_handoff, proposal_node_id, updated -- no content/details), ordered by wi_number, as {items, total, limit, offset, omitted}. `omitted` is {closed, archived} -- the rows the defaults hid, so a narrowed list can never be mistaken for the whole corpus. `updated` is when the item last changed (korg #1411): the staleness signal a sweep needs without one get_work_item per row -- wi_number already proxies creation age, this is the last-touched half. Defaults: everything not terminal (`open`, `resolved`, `done`, `parked`) and unarchived. Pass wi_status:\"all\" (or one status, including \"closed\") for the rest, `project` (name) to scope to one project. There is no detail flag: reading one item in full is get_work_item, which also inlines its comments and edges."),
        tool::<ops::NodeId>("get_item", "Fetch ANY node by node_id WITHOUT knowing its kind -- the read for when you are holding a bare id and would otherwise guess. Returns {kind, item}, where `kind` is one of the nine node kinds and `item` is EXACTLY the payload that kind's typed read returns (get_item on a work item returns get_work_item's body, on a program get_program's, and so on). `kind` travels with the payload because you came here not knowing it; do not sniff it from which fields are present. isError `not_found` ONLY when the id names no node at all -- which, unlike a typed read's not_found, is unambiguous, because every kind was tried. PREFER THE TYPED READ WHEN YOU KNOW THE KIND: get_work_item, get_proposal, get_program and the rest document what you expected, so a wrong id fails where the mistake is instead of quietly returning a different kind of thing. This tool exists for the other case, and that case is real: a session burned get_work_item then get_proposal before get_program answered for node 2440, with both failures indistinguishable from a bad id."),
        ...
        tool2::<ops::WiNumber, WorkItemPatch>("update_work_item", "Partially update a work item by its wi_number; returns the updated row (isError with code `not_found` if the wi_number does not exist). Only the fields you pass are changed. Status lifecycle: open -> resolved (implemented; may still need a user test or PR) -> done (agent satisfied; terminal but visible in default lists) -> closed (reserved for Ken; hidden from list_work_items unless you pass wi_status \"closed\" or \"all\" -- do not set unless directed). `parked` is off that line entirely (korg #810): deferred indefinitely, still visible in the default list because the point is keeping it in view, and never finished -- a dependency on a parked item stays an unmet blocker. For nullable fields (project_id, details, sprint, area_id, parent, category, check_after, invalidated_if) pass null to clear or omit to leave unchanged. `check_after` (a `YYYY-MM-DD` date -- the earliest an item's evidence can be judged) and `invalidated_if` (free text -- what state, if it changes, VOIDS the test) are the SOAK fields (korg #2153): null on ordinary work, both REQUIRED before a program may `soaks` this item, and independently clearable, because a soak whose window moves has not stopped being invalidated by the same condition. Moving projects (project_id) clears an area that no longer belongs to the target project unless you pass a valid area_id in the same call."),
        tool::<ops::WorkItemFlow>("work_item_flow", "korg's one time-series read (#1318): daily work-item flow, one row per day in the board's timezone, oldest first, ending today -- {day, added, closed, backlog, added_durable, closed_durable} per row, plus {backlog_before, horizon, timezone, durable_after_days, generated} on the envelope. `added` counts creations, from node.created (complete for every item; creating writes no transition row, so a log-derived added would be silently zero). `closed` counts moves to `closed`, from the transition log. `backlog` is the open (non-closed, unarchived) count at that day's end, reconstructed from the log -- today's value equals list_work_items' `total`. The `*_durable` split is where the signal is: an item is durable once it lived (or has so far lived) more than `durable_after_days` days; everything faster is sprint-internal churn, which is 3 of every 4 closes, so the raw added/closed pair reads ~1:1 forever and says nothing about the backlog. Note `added_durable` is structurally zero for the newest `durable_after_days` days -- durability is only knowable in retrospect, so at the 10-day default a summed `added_durable` covers only the OLDEST part of the window while a summed `closed_durable` covers all of it; printing the two side by side without saying so compares unlike figures. `backlog_before` is the open count at the end of the day BEFORE `days[0]` -- the window's baseline, and the one number the series cannot supply, because every row's `backlog` is that day's END. Take the window delta as `days.last().backlog - backlog_before`, NOT as `days.last().backlog - days[0].backlog`: the latter measures one day less than the `added`/`closed` sums do and is off by the first day's own net. It is null when the window starts at `horizon` (no prior day the log can answer) -- null, never 0, since a 0 is a real backlog level. `days` defaults to 10. A window reaching past `horizon` (where the transition log begins; it was deliberately not backfilled) is CLAMPED: the series starts at the horizon and is simply shorter than asked -- days the log cannot answer are absent, never zero-filled, because a zero that means 'no data' renders exactly like 'nothing closed'. Take the series length from the response, not from the default you passed."),
        ...
        tool::<ops::NodeId>("list_comments", "List the comments on a node -- ANY kind, not just work items and cards -- oldest first. Since sprint 055 every kind's UI surface renders them, so a proposal's, program's or report's thread is here too. Unpaginated: this is the whole thread, which is what get_work_item's inlined-comment cap points you at. Each comment carries `origin` (korg #1879) -- the writer's own self-reported label, unverified, and NULL on anything written before provenance existed or by a writer that sent none. Read it as a hint about who wrote a comment, never as proof."),
        tool2::<ops::NodeId, ops::CommentBody>("add_comment", "Add a comment to a node of any kind. Returns the created comment; isError with code `not_found` if the node does not exist. Optionally pass `origin` -- self-reported provenance (korg #1879), the same convention `relate` carries: send your skill name, so a reader of the thread can tell an overseer's clearance from a leg's note. It is recorded, NOT verified -- korg is no-auth HTTP on the fleet, so this is an audit aid and never a control. Absent stores NULL, which honestly means unknown rather than anonymous."),
        tool::<ops::Id>("delete_comment", "Delete a comment by its id. Returns {deleted: bool} — false means there was no such comment."),
        tool2::<ops::Id, ops::CommentBody>("update_comment", "Edit a comment's body by its id (from list_comments). `created` is preserved; `updated` advances. `origin` (korg #1879) is re-stamped only if you send one -- an editor that identifies itself takes the stamp, because it wrote the body that is now there; an edit that sends nothing leaves the existing origin alone rather than clearing it."),
        ...
        tool::<ops::Relate>("relate", "Create a relationship edge between any two nodes. The label reads left-to-right. The vocabulary is CLOSED -- these labels and no others: `covers` (proposal -> work item), `includes` (program -> proposal), `finding` (report -> work item), `depends_on` (dependent -> dependency), `materializes` (schedule -> work item: a schedule's generation history, normally written by materialize_schedule -- `last_wi_id` points only at the newest, the edges are the record of every one), `has_handoff` (node -> handoff; normally written by create_handoff, but relate attaches an existing handoff to another node), `has_attachment` (node -> attachment: what makes an uploaded image BELONG to something -- an attachment with no such edge is `pending` and the sweeper deletes it 24h after upload, so writing this edge rescues an image and unrelating one lets it go), and `soaks` (program -> work item: the program's terminal array of EXTENDED TESTS -- the work whose acceptance only the passage of days can satisfy, korg #2152) are DIRECTED -- orientation is meaningful, and the reverse is a distinct edge (A depends_on B plus B depends_on A is a cycle, not a duplicate). `related-to` (the two nodes are related) and `collides-with` (same contract / fold on landing -- the kfdc curator's mined collisions, korg #1002) are UNDIRECTED -- orientation is stored but meaningless, so read them symmetrically. An unregistered label is invalid_input naming the registry and the near-miss. Endpoint kinds are validated wherever the registry pins them: `covers`, `includes`, `finding`, `materializes` and `soaks` pin BOTH ends, `has_handoff` pins its right end (it must be a handoff) and `has_attachment` its right end (an attachment); `covers` additionally requires BOTH ENDS IN THE SAME PROJECT -- a proposal bundles work from one project, and cross-project work belongs to a program, the layer above proposals (korg #968). `includes` is the layer where cross-project work is legal, so it has NO same-project rule; `depends_on` across projects likewise stays legal and is the normal case, and `soaks` has none either (a program has no project at all). `soaks` carries TWO REFUSALS of its own, both invalid_input naming the cause. (1) The work item must already have `check_after` AND `invalidated_if` set -- set them with update_work_item first; this is what keeps a soak's invalidation condition from being the part everybody skips. (2) The work item must NOT still be covered by a LIVE proposal (`proposed`/`active`/`parked`) -- extracting a test out of a slice is `unrelate` the covers edge, THEN `relate` soaks, two calls on purpose, because one of them un-claims a proposal's work and that must never happen as a side effect. Exact duplicates dedup, and relating the reverse of an undirected edge returns the existing one. Optionally pass `rank` -- the position of the right endpoint within the left one, which is how a program's slices are ordered; re-relating an existing edge WITH a rank moves it in place and keeps its provenance, and re-relating without one leaves the position alone (so reordering never needs unrelate + relate). Optionally pass `origin` -- self-reported provenance (e.g. your skill name); it is recorded, not verified. Both endpoints must exist (isError `not_found`) and must differ (isError `invalid_input` -- self-edges are rejected)."),
        ...
        tool::<ops::ListReports>("list_reports", "List daily reports, newest first (summary fields only). Pass `source` to filter -- there is no report `class`, because the SOURCE value is the class (a soak scan files under `kfo-soak`). Pass `reviewed` to filter on whether somebody has acted on the report (korg #2154): omit for both (the default, so nothing an existing caller sees changes), `false` for what still wants attention, `true` for the audit view. Every row carries `reviewed`."),
        tool::<ops::NodeId>("get_report", "Fetch one report by node_id: full body plus linked finding work items, and `reviewed` -- whether somebody has acted on it (korg #2154). isError with code `not_found` if there is none."),
        tool::<ops::ReviewReport>("review_report", "Mark a report reviewed, or put it back (korg #2154). Returns the updated report; isError `not_found` if that node is not a report. ONE field, deliberately: rewriting a report's CONTENT is create_report, which upserts on (source, report_date) and replaces the finding edges with it. Mark it when you have ACTED on what it says -- done the ops action, filed the work item, judged the soak -- not when you have merely read it; the flag is what lets an operations pane stop showing a report without anyone closing or deleting it. A same-day re-run of the report RESETS this to false, because the content is new and a review of the text it replaced does not carry forward."),
        tool::<ops::ListProposals>("list_proposals", "The sprint queue: parked last, then pinned first, then rank, then node_id (a stable order -- equal ranks no longer shuffle between calls). Returns {items, omitted} -- NOT a bare array; `omitted` counts the rows the defaults hid ({done, declined, archived}), so a narrowed queue can never be mistaken for the whole corpus. Defaults: live proposals only (`proposed` + `active` + `parked`), unarchived, and the lean projection -- node_id, title, status, project, rank, pinned, covered_count, comment_count, no `summary`. Pass status:\"all\" (or one status) for the rest, detail:\"full\" for every column including `summary` and `notes`. `parked` (korg #1534) is DEFERRED INDEFINITELY, not declined and not hidden: korg keeps it in the default read on purpose and sorts it below the line -- there is no `omitted` field for it, because nothing was hidden. It is the answer for work blocked with no end date, where `declined` would say the opposite (a decision not to do it). Filter it out yourself if your surface wants only what can be started now. Reading one proposal is get_proposal, which also gives you the work items it covers."),
        ...
        tool2::<ops::NodeId, ProposalPatch>("update_proposal", "Partially update a sprint proposal by its node_id; returns the updated proposal (isError with code `not_found` if that node is missing or is not a proposal). Only the fields you pass are changed. Use this for status transitions (proposed -> active -> done/declined), reordering (rank), pinning, or archiving. `parked` (korg #1534) is off that line entirely: a sideways move from either live status meaning deferred until some condition fires, still live, still blocking anything that depends on it, and still visible -- sorted below the line rather than hidden. Un-parking is NOT reversible by korg: the status a row was parked out of is not recoverable from the row, so you must name the status to return it to. `summary` is capped at 500 characters (invalid_input above that) -- long-form analysis belongs in `notes`, which is unbounded; pass null to clear it."),
        ...
        tool::<ops::NodeId>("get_program", "Fetch one program by node_id with the whole rollup, so a consumer NEVER crawls program -> proposals -> work items: the program fields, `span` (its derived projects), and `slices` -- the included proposals in program order, each with node_id, title, status, project, its position `rank`, `covered_count`, and one work-item count per status (open/resolved/done/closed/parked -- they sum to `covered_count`). It also carries `soaks` (korg #2152) -- the program's terminal array of EXTENDED TESTS in rank order, each {node_id, wi_number, title, project, wi_status, check_after, invalidated_if, rank}. That is the whole content of a `soaking` program: by the time one may enter that state every slice is terminal, so the soaks are the only part left to render or judge. `check_after` is `YYYY-MM-DD`, the earliest the evidence can be judged; `invalidated_if` says what voids the test, and a scan should check it FIRST, before asking whether the date has arrived. Empty for the great majority of programs. Also carries inlined comments and the `related` block, where a `has_handoff` edge surfaces -- a program is precisely the kind that accrues one, and it is required context. (`includes` and `soaks` edges are NOT repeated in `related`; they are `slices` and `soaks`.) isError with code `not_found` if there is none."),
        tool::<ops::ListPrograms>("list_programs", "List programs, parked last, then pinned first then rank then node_id. Returns {items, omitted} -- NOT a bare array; `omitted` is {done, archived}, the rows the defaults hid. Defaults: live programs only (`queued` + `active` + `holding` + `soaking` + `parked`), unarchived. Pass status:\"all\" (or one status) for the rest. There is no detail flag and no project filter: a program has no project of its own (its `span` is derived and is on every row), and with a 500-character `aim` the full row is already the lean one. Four of the six statuses exist because \"active\" alone would lie in several directions. `queued` (korg #1424) means NO slice has started -- a program is born there, and korg promotes it the moment any slice does, so you may read it as a fact about the slices rather than reconstructing one. `holding` means started but nothing currently in flight -- it is the state a program spends most of its life in, and it is deliberately not collapsed into `active`. `soaking` (korg #2151) means all slice work is DONE and acceptance is waiting on extended tests that only the passage of time can satisfy -- read it as \"finished, on a clock\", and get its `soaks` array from get_program. It is LIVE and stays in every default read on purpose: a soaking program is precisely the one somebody must still act on, and moving it to a calmer panel is the consumer's job, never korg's. `parked` (korg #1535) is the one status that is NOT on the lifecycle: it means dormant with no end date, and unlike `queued` it is DECLARED rather than derived -- korg never sets or clears it, nothing promotes a parked program, and a parked program may legitimately hold `active` slices. Read it as the operator overriding what the slices imply. It stays in the default list, sorted below the line."),
        tool2::<ops::NodeId, ProgramPatch>("update_program", "Partially update a program by its node_id; returns the updated program (isError `not_found` if that node is missing or is not a program). Only the fields you pass change: title, aim, notes, status (active -> holding -> soaking -> done, plus `parked` off that line -- dormant with no end date, declared by you and never by korg, and un-parking requires naming the status to return to because korg will not guess it), rank, pinned, archived, tags. Entering `soaking` (korg #2151) is korg's ONE transition rule on a program, because unlike every other status it asserts something about the slices: it is invalid_input unless EVERY slice is terminal (`done`/`declined`) AND the program has at least one `soaks` edge to an unfinished work item. Leaving it is unconstrained in all three useful directions -- `done` is the explicit close, `holding` is a failed soak, and `active` is the new slice that fixes it (korg makes that last one itself, lifting a soaking program the moment a slice starts, exactly as it lifts a `queued` one). `aim` is capped at 500 characters. Slice membership and ORDER are not here -- use relate/unrelate with the `includes` label, passing `rank` to position a slice."),
        tool2::<ops::NodeId, ops::SetAwaiting>("set_awaiting", "Mark a node -- ANY kind: work item, proposal, program, card -- as awaiting Ken, or clear that marker. This is how an agent says \"this moves only when Ken acts\": a decision, an ops action no agent can perform, a review. Pass awaiting:true with a `note` saying what is being asked; pass awaiting:false to clear it, which you SHOULD do yourself once you have the answer rather than leaving an answered ask in the lane. Re-marking a node that is already awaiting KEEPS the original timestamp and only updates the note -- the age of an ask is what makes the lane useful, so it is not restarted. The marker is a real column, not a tag: tags are written wholesale and an unrelated tag edit would silently clear it. Clearing also happens automatically when Ken himself acts (a work item reaching `closed`, a proposal reaching `done`/`declined`, a program reaching `done`, a card reaching `Done`/`Cut` -- the kanban is his board end to end -- and anything archived)."),
        tool::<ops::NoArgs>("list_awaiting", "Everything currently waiting on Ken, oldest ask first -- one call, every node kind. Each row carries node_id, kind, wi_number (work items), a resolved title, project, the node's OWN status, `awaiting_since` and `awaiting_note`, so a board renders the lane without a follow-up read per row. Archived nodes and ones in a state only Ken sets (closed work items, `done`/`declined` proposals, done programs, `Done`/`Cut` cards) are excluded -- the lane holds live asks, never answered ones. Note that `resolved` and `done` work items DO appear: \"implemented, needs your user test\" is the canonical awaiting-Ken state."),
        tool::<ops::Search>("search", "Full-text search across the WHOLE corpus in one call (korg #1177): every node kind that carries prose -- work items, proposals, programs, cards, reports, handoffs, schedules, links -- plus one document per COMMENT, which is where the answer usually is. A resolution comment, a review verdict, a recon note: none of that is reachable by scanning titles, and title-scanning is measurably the worse tool (9/12 vs 12/12 on the frozen acceptance suite). Reach for this INSTEAD of paging list_work_items and reading rows yourself.\n\nReturns {items, total, limit, offset, omitted, relaxed, parsed}. Each item is lean and routable: node_id, comment_id (set only for comments, so \"this item matched\" is distinguishable from \"someone said this on it\"), kind, `locator` (`WI-836`, `korg:1395`, `korg:1395#comment-777` -- the spelling Ken and skills already use), title, status, project, `score`, `updated`, and a bounded `snippet` centred on the matched terms rather than the head of the document. Bodies are NOT here by design -- follow the locator with get_work_item / get_proposal / get_report when the snippet is not enough.\n\nQuery semantics: ALL terms are required. When that matches nothing the query is relaxed to ANY term and `relaxed: true` says so -- always read that flag, because a relaxed answer is a broad one and its `total` counts any-term matches (frequently most of the corpus). Quoted \"exact phrases\" and a leading `-` to exclude both work; a query carrying an exclusion is NEVER relaxed, because `foo -bar` relaxed would invert what you asked for. `parsed` shows the tsquery Postgres actually ran (stemming applied, stop words dropped), which is how you diagnose a surprising result set without guessing.\n\nScoping: by default only LIVE rows are searched -- each kind's own terminal state is excluded, matching what its list read hides (`closed` work items, `done`/`declined` proposals, `done` programs, `Done`/`Cut` cards), and archived rows are excluded too. `omitted` is {terminal, archived}, counted as a cascade so nothing is double-counted, and it is how you know a narrow answer was narrowed. **Pass scope:\"all\" whenever you are chasing something that was decided and closed** -- which is most historical questions; the default is tuned for \"what is in flight\", not \"what did we conclude\". Narrow further with `kind` (one document kind) and `project` (a comment inherits its parent's project).\n\nRanking is fixed and deliberately not a knob. This is a LEXICAL search: it matches words, not meaning, so a query sharing no vocabulary with its answer will miss -- phrase it in the words the record would use."),
        tool::<ops::NoArgs>("get_board", "The whole board in ONE call (korg #970) -- what a dashboard needs, instead of the 17-call crawl the 2026-07-31 backlog review had to do. Takes NO arguments: it is one screen's state, and every filter it could offer is already decided by what the panels are. Returns {generated, active, queue, proposals_omitted, proposal_edges, blocked, programs, programs_omitted, awaiting, depth, reports, events, sources, due_schedules, in_flight_schedules}. `active` and `queue` are the `active` and `proposed` proposals, pinned-first-then-rank, with `parked` proposals (korg #1534) carried at the END of `queue` rather than in a bucket of their own -- they are live, so korg shows them; `status` says which they are, and a surface that wants only startable work filters on that literal, each with `summary`, a work-item rollup (covered_count plus one count per status: open/resolved/done/closed/parked, which sum to it) so progress renders without a call per proposal, and `synopsis` -- the newest comment opening with the \u{27e6}curator\u{27e7} marker (korg #1003, the machine-curated line; null when none). `proposals_omitted` is the usual {done, declined, archived}. `proposal_edges` (korg #1003) are every edge whose BOTH endpoints are active/queue rows -- {left, right, label, directed (from the registry -- read undirected labels symmetrically), origin, created}, the read surface for edge provenance, which is how curated edges (origin \"kfdc-curator\") stay distinguishable from human ones. `programs` are the live programs, each with its ordered `slices` exactly as get_program returns them -- parked programs last, on the same rule as the queue. `blocked` (korg #978) is the DETERMINISTIC half of Deconfliction: every UNMET depends_on holding up a live row, one entry per (row, blocker) pair -- {proposal, via, dependent, dependent_wi_number, blocker, blocker_kind, blocker_wi_number, blocker_title, blocker_project, blocker_status, sequenced_by}. `via` is \"proposal\" when the row itself carries the edge and \"covered\" when one of its covered work items does; both count. Unfinished is derived from the vocabulary per kind, so a `done` work item does NOT block (that is the completion split, not the list-visibility one). One hop only, and an archived blocker does not block -- a row held blocked forever by something nobody will ever do is worse than one that frees up. `sequenced_by` names the live program whose ordered slices ALREADY express this dependency (kfdc #1070), so a render can drop what Operations is drawing anyway instead of showing the same data twice; null when there is none. `events` (korg #977) is the ticker: the newest 20 status transitions, newest first -- {node_id, kind, wi_number, title, project, from_status, to_status, at}. A write that does not CHANGE the status produces no event, which is the whole point: `node.updated` advances on any edit, so a feed built from it would date a proposal by its last tag edit. The log starts at migration 0026 and was deliberately NOT backfilled (there was no honest history to backfill from), so an empty ticker means \"nothing has moved since the migration\", never \"nothing has ever moved\". `awaiting` is list_awaiting's lane, `depth` is per-project queue depth (every project, with its `status` and `category` -- `category` is there so a consumer can drop the permanently-active `eval` residue bucket without a second read), `reports` the newest 5, and `sources` (korg #950) is every reporting source with its `freshness` (fresh|stale|retired|unrated|on-demand) and what it `asserts` (ok|attention|problem|unknown) -- anything not `fresh` asserts `unknown`, never a last known status, which is the whole point: a stale GREEN is how korg lied about the fleet for 11 days in July 2026. `sources` is deliberately UNCAPPED where `reports` is 5 -- a cap could push a stale source off the board behind fresher ones, which inverts the failure it exists to catch. `due_schedules` (korg #1385) is what is DUE RIGHT NOW, soonest-due first: exactly `list_schedules(due_only:true)`'s rows, so due-ness keeps its single definition and the board inherits all three clauses -- active, interval elapsed, and no outstanding materialized item (a drill whose work item is already open does not nag from here). Uncapped for the same reason `sources` is: the row a cap would drop is the one that has waited longest. It exists because schedule 1112 sat due for nine days where nothing that gets looked at daily could say so. `in_flight_schedules` (korg #1644) is its other half: every schedule whose newest materialized work item is still UNFINISHED, newest firing first, each row {node_id, title, project, wi_number, wi_title, wi_status, materialized_at}. It exists because materializing ENDS due-ness -- correctly -- and the open item left behind landed in no panel at all: not in a proposal, not blocked, not awaiting, and `events` carries status changes, so being created was never one. It is a SIBLING of `due_schedules` and not a `state` discriminator folded into it, so a consumer already rendering due-ness needs no change. Unfinished is the vocabulary's set, so `parked` counts (#810). Deliberately NOT filtered to active schedules: a `once` schedule marks itself `done` as it fires, and its open item is exactly what must not vanish -- unfinished-ness is a fact about the item, so the item is what is filtered on. Uncapped, bounded by construction: a schedule leaves the block when its item is finished. There is deliberately NO counters block -- live proposals is active+queue, shipped is proposals_omitted.done, awaiting is awaiting.length, projects is depth filtered by status; a counter that can disagree with the list beside it is a bug. `generated` is Postgres's clock, the same one every timestamp here came from, so ages computed against it are right. For one proposal's covered items, one program's detail, or anything paged, use that entity's own read."),
        tool::<NewSchedule>("create_schedule", "Create a SCHEDULE -- a work-item template plus a cadence, for work that a DATE makes appear (korg #581). This is korg's answer to \"recheck this after the DST transition\" and \"the restore drill is due again\", neither of which could be filed before: prose on a done node is out of every default view, and a work item for something not due for three months makes the queue lie. `cadence` is once|weekly|fortnightly|monthly|quarterly|yearly -- `once` is not a special case, it is the cadence whose interval is ZERO, so its `anchor_at` simply IS its fire date. The set mixes two families and the distinction decides which value is right: `weekly` and `fortnightly` are whole WEEKS, so they preserve a weekday; every calendar-month cadence drifts off it (2026-08-08 is a Saturday, 2026-09-08 is a Tuesday). \"Every other Saturday\" is therefore `fortnightly`, never `monthly`. `anchor_at` is where the interval counts from: SEED IT with the real last-done date (a quarterly drill last run 2026-07-08 anchored there comes due 2026-10-08, not today). `anchor_mode` picks which event advances it -- `completed` (default; when the materialized work item reaches done/closed) or `created` (at materialization). `title` and `template` may use exactly these substitutions: {YEAR}, {MONTH}, {DAY}, {DATE}, {QUARTER}; any other {PLACEHOLDER} is invalid_input rather than rendering literally into a title months from now. Creating a schedule materializes NOTHING -- korg runs no scheduler; call materialize_schedule when the work starts."),
        ...
        tool::<ops::ListSchedules>("list_schedules", "List schedules, SOONEST-DUE FIRST -- the first row is the next thing that wants doing. Returns {items, omitted} -- NOT a bare array; `omitted` is {done, archived, not_due}, and `not_due` is what a due_only read hid, so \"nothing is due\" is never confusable with \"I filtered it out\". Defaults: live schedules (`active` + `paused`), unarchived. Pass due_only:true for just what is due now. `due` is computed on every read, never stored -- a stored due flag is a cached predicate over a clock, which is exactly the thing that goes stale silently. A schedule is due when it is active, its interval has elapsed since `anchor_at`, AND its last materialized work item is no longer unfinished (`open`, `resolved`, `parked`) -- while that item is unfinished, IT is the surface and the schedule does not compete with it. `parked` counts as outstanding on purpose: parking a materialized drill defers it, and re-firing the schedule would produce a duplicate of work somebody just deliberately deferred."),
        ...
        tool::<ops::NoArgs>("list_report_sources", "Every known report source and whether korg can currently BELIEVE it (korg #950) -- most-alarming first. A source that filed daily and then stopped is itself the alert: in July 2026 kmon crashed 1s into collection for 11 straight days and korg kept serving its last GREEN, so the fleet read healthy BECAUSE the thing checking it was broken. Each row carries `freshness` (fresh|stale|retired|unrated|on-demand) and `asserts` (ok|attention|problem|unknown). THE RULE: anything not `fresh` asserts `unknown` -- never the last known status, because restating a stale GREEN is the exact failure this ends. There is deliberately no last_status field to reach for. `unrated` means fewer than 3 reports so no cadence can be inferred and none was declared; declare one with set_report_source to skip that state. `on-demand` is the OPPOSITE claim and not a weaker one: somebody declared this source has NO cadence at all -- it files on an event, so its silence never means anything and more reports will never promote it. Cadence is the median gap between a source's own report_dates unless declared; grace defaults to the cadence (floor 1 day), so a daily source goes stale on the third silent day."),
        tool2::<ops::SourceName, ReportSourcePatch>("set_report_source", "Declare a report source's expected cadence, its grace window, or its retirement (korg #950). Every field is an OVERRIDE -- omit one and korg keeps deriving it from the source's own report history; pass null to clear an override back to derivation. `retired:true` is how a deliberately-ended source stops nagging WITHOUT also silencing a broken one: it is an explicit declaration, so \"we turned this off\" stays distinguishable from \"this died\", which silence alone can never do. `on_demand:true` is the fourth declaration and the one for a source with NO cadence BY DESIGN -- a report-on-change scanner, an interactive tool run when somebody runs it. It pins freshness to `on-demand`, skips inference entirely (so no amount of history can promote it) and never alerts. Reach for it instead of `retired` whenever the source has not ended, and instead of leaving it undeclared: `unrated` is honest only until inference notices a pattern, and `kyac` spent 64 days quietly earning a 2-day cadence nobody declared before korg started calling it late. `on_demand` and `cadence_days` are CONTRADICTORY claims, not two settings -- a call that would leave a source carrying both is invalid_input, including one that assembles it over two calls, and the error names which one to drop. You may declare a source BEFORE its first report -- that is how a new daily source skips the `unrated` state on day one instead of spending three days in it. Returns the source's freshness row."),
        ...
        tool2::<ops::ProjectName, ProjectPatch>("update_project", "Update a project's metadata by name (the name itself is immutable), returning the updated project: status (active|archived — those are the only two, and the question they answer is \"is this a legitimate target for new work?\"), machines, deploy_to, category, description, notes, gh_repo, src_path, starred. Omitted fields are unchanged. `description` is capped at 160 characters — it is a routing line, not a blurb; put anything longer in `notes`. `starred` marks a project hot for the week, which is what lifts it into the band at the top of korg's project rails — it orders nothing and gates nothing, and is not the `pinned` that sprint proposals and programs carry. ARCHIVING IS NOT JUST A STATUS (korg #3003): a patch setting status `archived` also clears `src_path`, `machines` and `deploy_to` in the same transaction, because all three answer \"where is the source\" and for an archived project the answer is \"nowhere\". Nothing is lost — the cleared values are appended to `notes` first — and setting any of them in the same call will not survive it. Coming back is unaffected: `update_project` is deliberately outside the refusal that stops archived projects taking new work, so setting status `active` and re-locating the row in one call works, and clears nothing."),
...
impl KorgServer {
    pub async fn call(
        ...
        match name {
            ...
            // --- kind-agnostic read (#2446) ---
            "get_item" => {
                let a: ops::NodeId = parse_args(args)?;
                let kind = match repo::node_kind_of(pool, a.node_id).await {
                    Ok(Some(k)) => k,
                    // The one not-found this tool can emit, and it is
                    // unambiguous in a way the typed reads' is not: every kind
                    // was tried, so the id names no node at all.
                    Ok(None) => {
                        return Ok(not_found(format!("no node with node_id {}", a.node_id)))
                    }
                    Err(e) => return Ok(to_err(e)),
                };
                // `kind` travels with the payload rather than being left for the
                // caller to sniff from which fields happen to be present. The
                // caller reached for this tool precisely because it did not know
                // the kind; handing back a bare typed body would make it
                // reconstruct the one fact it came here missing, which is GP-13
                // in the korg+ plan and #2446's actual complaint.
                macro_rules! enveloped {
                    ($read:expr) => {
                        match $read.await {
                            Ok(Some(v)) => match serde_json::to_value(v) {
                                Ok(item) => ok_json(serde_json::json!({
                                    "kind": kind,
                                    "item": item,
                                })),
                            ...
                            // `node` says this kind, the satellite table has no
                            // row: korg's own data is inconsistent, and saying
                            // "no such node" would blame the caller for it.
                            Ok(None) => Ok(err_with_code(
                                format!(
                                    "node {} is a {kind} in `node` but has no {kind} row — \
                                     korg's data is inconsistent, not your id",
                                    a.node_id
                                ),
                                ErrorCode::Internal,
                            )),
                ...
                match kind.as_str() {
                    "workitem" => enveloped!(repo::get_work_item_detail(pool, a.node_id)),
                    "card" => enveloped!(repo::get_card(pool, a.node_id)),
                    "link" => enveloped!(repo::get_link(pool, a.node_id)),
                    "sprint_proposal" => enveloped!(repo::get_proposal_detail(pool, a.node_id)),
                    "report" => enveloped!(repo::get_report(pool, a.node_id)),
                    "handoff" => enveloped!(repo::get_handoff(pool, a.node_id)),
                    "program" => enveloped!(repo::get_program_detail(pool, a.node_id)),
                    "schedule" => enveloped!(repo::get_schedule_detail(pool, a.node_id)),
                    "attachment" => enveloped!(repo::get_attachment(pool, a.node_id)),
                    // Unreachable while `get_item_covers_every_node_kind` passes
                    // — it fails the build on a kind added to NODE_KINDS without
                    // an arm here. Kept as an honest error rather than a panic,
                    // because the fence lives in a test and a test can be
                    // skipped; this is what a skipped fence should cost.
                    other => Ok(err_with_code(
                        format!(
                            "node {} has kind '{other}', which get_item has no read for — \
                             this is a korg bug, not a bad id",
                            a.node_id
                        ),
                        ErrorCode::Internal,
                    )),
            ...
            "add_comment" => {
                ...
                respond(repo::add_comment(pool, a.node_id, &b.body, b.origin.as_deref()).await)
            }
            "update_comment" => {
                let (a, b) = parse_args2::<ops::Id, ops::CommentBody>(args)?;
                respond(repo::update_comment(pool, a.id, &b.body, b.origin.as_deref()).await)
            }
            "delete_comment" => {
            ...
            "list_reports" => {
                ...
                respond(repo::list_reports(pool, a.source.as_deref(), a.reviewed, a.limit).await)
            }
            "review_report" => {
                let a: ops::ReviewReport = parse_args(args)?;
                respond(repo::set_report_reviewed(pool, a.node_id, a.reviewed).await)
            }
            "get_report" => {
                let a: ops::NodeId = parse_args(args)?;
            ...
            "search" => {
                let a: ops::Search = parse_args(args)?;
                respond(repo::search(pool, a.into()).await)
            }
            ...
            "get_attachment" => {
                ...
                match a.resolve() {
                    ...
                    Ok(node_id) => respond_found(repo::get_attachment(pool, node_id).await, || {
                        repo::attachment_not_found(node_id)
                    }),
                }
            }
            "list_attachments" => {
