...
pub fn tools() -> Vec<Tool> {
    vec![
        ...
        tool::<ops::NodeId>("get_program", "Fetch one program by node_id with the whole rollup, so a consumer NEVER crawls program -> proposals -> work items: the program fields, `span` (its derived projects), and `slices` -- the included proposals in program order, each with node_id, title, status, project, its position `rank`, `covered_count`, and one work-item count per status (open/resolved/done/closed/parked -- they sum to `covered_count`). Also carries inlined comments and the `related` block, where a `has_handoff` edge surfaces -- a program is precisely the kind that accrues one, and it is required context. isError with code `not_found` if there is none."),
        tool::<ops::ListPrograms>("list_programs", "List programs, parked last, then pinned first then rank then node_id. Returns {items, omitted} -- NOT a bare array; `omitted` is {done, archived}, the rows the defaults hid. Defaults: live programs only (`queued` + `active` + `holding` + `parked`), unarchived. Pass status:\"all\" (or one status) for the rest. There is no detail flag and no project filter: a program has no project of its own (its `span` is derived and is on every row), and with a 500-character `aim` the full row is already the lean one. Three of the five statuses exist because \"active\" alone would lie in several directions. `queued` (korg #1424) means NO slice has started -- a program is born there, and korg promotes it the moment any slice does, so you may read it as a fact about the slices rather than reconstructing one. `holding` means started but nothing currently in flight -- it is the state a program spends most of its life in, and it is deliberately not collapsed into `active`. `parked` (korg #1535) is the one status that is NOT on the lifecycle: it means dormant with no end date, and unlike `queued` it is DECLARED rather than derived -- korg never sets or clears it, nothing promotes a parked program, and a parked program may legitimately hold `active` slices. Read it as the operator overriding what the slices imply. It stays in the default list, sorted below the line."),
        tool2::<ops::NodeId, ProgramPatch>("update_program", "Partially update a program by its node_id; returns the updated program (isError `not_found` if that node is missing or is not a program). Only the fields you pass change: title, aim, notes, status (active -> holding -> done, plus `parked` off that line -- dormant with no end date, declared by you and never by korg, and un-parking requires naming the status to return to because korg will not guess it), rank, pinned, archived, tags. `aim` is capped at 500 characters. Slice membership and ORDER are not here -- use relate/unrelate with the `includes` label, passing `rank` to position a slice."),
